...
public class Hypothec extends Credit {
    ...
    public Credit create(Bank bank, Client client, double sum, int creditTerm) {
        if (sum <= 300000){
            ...
            return new Credit(bank,client.getMobilePhone(),TypeOfCredit.HYPOTHEC.type + "_1",  sum,25.9,creditTerm);
        }else if (sum > 1000000 && sum <= 2300000 ){
            return new Credit(bank,client.getMobilePhone(),TypeOfCredit.HYPOTHEC.type + "_2",  sum,19.9,creditTerm);
        }else if (sum > 2300000 && sum <= 3700000 ){
            return new Credit(bank,client.getMobilePhone(),TypeOfCredit.HYPOTHEC.type + "_3",  sum,17.9,creditTerm);
        }else if (sum > 3700000 && sum <= 5000000 ){
            return new Credit(bank,client.getMobilePhone(),TypeOfCredit.HYPOTHEC.type + "_4",  sum,15.9,creditTerm);
        }else if (sum > 5000000 && sum <= 10000000 ){
            return new Credit(bank,client.getMobilePhone(),TypeOfCredit.HYPOTHEC.type + "_5",  sum,13.9,creditTerm);
        }else if (sum > 10000000 && sum <= 20000000 ){
            return new Credit(bank,client.getMobilePhone(),TypeOfCredit.HYPOTHEC.type + "_5",  sum,12.9,creditTerm);
        }else {
            return null;
        }
    }
